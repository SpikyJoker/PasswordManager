...
freeconfig = { # all values held within can be changed completely freely
    ...
    'galois_matrix': [[]], # the matrix used for the MixColumns step
...
class Diffusion():
    ...
    galois_field = freeconfig['galois_matrix'] #This matrix is absolutely wild and uses some pretty deep maths, that i don't yet fully understand. Luckily, it's already been implemented in the MixColumn function, and it can be freely changed. https://www.youtube.com/watch?v=qGOoDZ100qY
    @staticmethod
    def galois_mult(a, b):
        ...
        for _ in range(8):
            if b & 1:
                p ^= a
            a <<= 1
            if a & 0x100: #https://realpython.com/cdn-cgi/image/width=500,format=auto/https://files.realpython.com/media/and.ef7704d02d6f.gif
                a ^= 0x11b
            b >>= 1
        return p
        # or operator looks like | https://realpython.com/cdn-cgi/image/width=500,format=auto/https://files.realpython.com/media/or.7f09664e2d15.gif
